...
export function areRequiredPreviewDestinationIdsLoaded(
  requiredPreviewDestinationIds,
  loadedPreviewDestinationIds
) {
  if (!requiredPreviewDestinationIds?.length) {
    return true;
  }

  if (!loadedPreviewDestinationIds?.length) {
    return false;
  }

  return requiredPreviewDestinationIds.every((destinationId) =>
    loadedPreviewDestinationIds.includes(destinationId)
  );
}
