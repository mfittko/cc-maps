...
export function createClearedRoutePlan(routePlan, selectedDestinationId) {
  const previousOwnerDestinationId = routePlan?.destinationId || selectedDestinationId;
  const nextOwnerDestinationId = selectedDestinationId || previousOwnerDestinationId;

  if (!previousOwnerDestinationId || !nextOwnerDestinationId) {
    return null;
  }

  return createRoutePlan(nextOwnerDestinationId, []);
}
...
export function getActiveRoutePlanStorageKey(storageKey) {
  return `${storageKey}:plan:active`;
}
...
export function readActiveStoredRoutePlan(storageKey) {
  if (typeof window === 'undefined') {
    return null;
  }

  try {
    const rawDestinationId = window.localStorage.getItem(getActiveRoutePlanStorageKey(storageKey));

    if (typeof rawDestinationId !== 'string' || !/^\d+$/.test(rawDestinationId)) {
      return null;
    }

    return readStoredRoutePlan(rawDestinationId, storageKey);
  } catch (error) {
    console.warn('Failed to read active stored route plan', error);
    return null;
  }
}
...
export function writeStoredRoutePlan(routePlan, storageKey) {
  ...
  try {
    ...
    window.localStorage.setItem(getActiveRoutePlanStorageKey(storageKey), routePlan.destinationId);
  } catch (error) {
...
export function clearStoredRoutePlan(destinationId, storageKey) {
  ...
  try {
    const key = getRoutePlanStorageKey(destinationId, storageKey);
    const activeKey = getActiveRoutePlanStorageKey(storageKey);

    window.localStorage.removeItem(key);

    if (window.localStorage.getItem(activeKey) === String(destinationId)) {
