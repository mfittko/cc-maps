...
class MockMap {
  constructor(options = {}) {
    ...
    if (typeof globalThis !== 'undefined') {
      globalThis.__ccMapsMockMap = this;
    }
  }
  ...
  emitLayerEvent(eventName, layerId, eventPayload) {
    const handlers = this.handlers.get(`${eventName}:${layerId}`) || [];
    handlers.forEach((handler) => handler(eventPayload));
  }
  ...
  queryTerrainElevation() {
    // When the mock is enabled, UI validation does not depend on terrain
    // variation, so it intentionally reports sea-level elevation everywhere.
    return 0;
  }
  ...
  remove() {
    ...
    if (typeof globalThis !== 'undefined' && globalThis.__ccMapsMockMap === this) {
      delete globalThis.__ccMapsMockMap;
    }
