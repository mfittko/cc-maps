import { decodeRoutePlanFromUrl, readActiveStoredRoutePlan, readStoredRoutePlan } from './route-plan';
...
export function isPlanningModeQueryValue(value) {
...
export function getPersistedRoutePlanSources(routeQueryValue, selectedDestinationId, storageKey) {
  const routeFromUrl = decodeRoutePlanFromUrl(routeQueryValue);
  const routeFromStorage = readActiveStoredRoutePlan(storageKey) || (selectedDestinationId
    ? readStoredRoutePlan(selectedDestinationId, storageKey)
    : null);

  return {
    routeFromUrl,
    routeFromStorage,
    persistedRoutePlan: routeFromUrl || routeFromStorage,
  };
}